...
int ialloc(int dev)
{
								...
								get_block(dev, imap, buf); //built in function

								for(i = 0; i < ninodes; i++)
								{
																if(tst_bit(buf, i) == 0) //function defined above
																{
																								set_bit(buf, i); //helper function
																								decFreeInodes(dev); //helper function, decreases number of free inodes
...
int balloc(int dev)
{
								...
								get_block(dev, bmap, buf); //dev is device

								for(i = 0; i < nblocks; i++)
								{
																if(tst_bit(buf, i) == 0) //helper function
																{
																								set_bit(buf, i); //helper function
																								decFreeInodes(dev); //helper function, decreases number of free inodes
